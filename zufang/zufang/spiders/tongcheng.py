...
class TongchengSpider(CrawlSpider):
    ...
    rules = (
        ...
        Rule(LinkExtractor(restrict_xpaths=('//div[@class="des"]/h2/a',)), callback='parse_item',
             follow=True),
    ...
    def parse_item(self, response):
        ...
        i['rent'] = response.xpath('//div[@class="house-pay-way f16"]/span[1]/b/text()').extract_first()
        ...
        i['area'] = re.findall('    (.*?)       ',house_type)[0] if len(re.findall('    (.*?)       ',house_type))>0 else None
        i['Decoration'] = re.findall('平  (.*?)$',house_type)[0] if len(re.findall('平  (.*?)$',house_type))>0 else None
        Orientation = response.xpath('//ul[@class="f14"]/li[3]/span[2]/text()').extract_first()
        i['Orientation'] =re.findall('(.*?)  ',Orientation)[0] if len(re.findall('(.*?)  ',Orientation))>0 else None
        ...
        i['metro_distance'] = response.xpath('//ul[@class="f14"]/li[5]/em/text()').extract_first()
        i['metro_distance'] = re.findall('\d+',i['metro_distance'])[0] if len(re.findall('\d+',i['metro_distance']))>0 else None
